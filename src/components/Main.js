...
const Main = () => {
  ...
  useEffect(() => {
    const fetchInactiveUsersCount = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:5000/api/inactiveUsersCount', {
          headers: {
  ...
  useEffect(() => {
    const fetchPendingPaymentCount = async () => {
      try {
        const token = localStorage.getItem('token'); // 로그인 토큰 가져오기
        const response = await axios.get('http://127.0.0.1:8865/api/order/pendingCount', {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  useEffect(() => {
    ...
    const fetchUnansweredCount = async () => {
      try {
        const token = localStorage.getItem('token'); // 로그인 토큰 가져오기
        const response = await axios.get('http://127.0.0.1:8865/api/qnaQuestion/unansweredCount', {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
